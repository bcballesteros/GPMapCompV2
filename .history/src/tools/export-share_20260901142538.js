...
function debugPreview(event, details = {}) {
    ...
    const entry = {
        ...
        mainCanvases: getDebugCanvasSummary(mainMap),
...
function getCanvasTransform(canvas) {
    ...
    const widthRatio = Number.isFinite(width) && canvas.width ? width / canvas.width : 1;
    const heightRatio = Number.isFinite(height) && canvas.height ? height / canvas.height : 1;
...
function bindPreviewSyncListeners() {
    ...
    const scheduleSync = (reason = 'unknown') => {
        ...
        if (!getRenderedElementRect(document.getElementById('mapPreviewMap'))) {
            return;
        }
    ...
    const watchLayer = (layer) => {
        layer?.on('change', () => scheduleSync('layer-change'));
        layer?.getSource?.()?.on('change', () => scheduleSync('source-change'));
    };
    ...
    mainMap.getLayers().forEach(watchLayer);
...
function createMainMapSnapshot(map) {
    const [width, height] = map?.getSize?.() || [];
    if (!width || !height) {
        return null;
    }

    const snapshot = createExportCanvas(width, height);
    ...
    return drawMapViewportToContext(map, context, width, height) ? snapshot : null;
}

function displayPreviewSnapshot(target, snapshot) {
    target.parentElement?.style?.setProperty('--export-preview-aspect-ratio', `${snapshot.width} / ${snapshot.height}`);
    const rect = target.getBoundingClientRect();
    const pixelRatio = window.devicePixelRatio || 1;
    const width = Math.max(1, Math.round(rect.width * pixelRatio));
    const height = Math.max(1, Math.round(rect.height * pixelRatio));
    const previewCanvas = createExportCanvas(width, height);
    ...
    context.fillStyle = '#ffffff';
    context.fillRect(0, 0, width, height);
    context.drawImage(snapshot, 0, 0, snapshot.width, snapshot.height, offsetX, offsetY, drawWidth, drawHeight);

    previewCanvas.className = 'export-preview-canvas';
    previewCanvas.setAttribute('aria-label', 'Current map snapshot');
    target.replaceChildren(previewCanvas);
    return true;
}
...
export async function renderMapPreview(reason = 'direct') {
    ...
    const hasMainMapSize = await ensureMapRenderedSize(mainMap);
    if (!hasMainMapSize) {
        return;
    }
    ...
    const snapshot = createMainMapSnapshot(mainMap);
    const previewDisplayed = snapshot && displayPreviewSnapshot(target, snapshot);

    if (!previewDisplayed || renderToken !== previewRenderToken) {
        debugPreview('renderMapPreview:exit-before-ready', {
            reason,
            renderToken,
            currentToken: previewRenderToken,
            previewDisplayed: Boolean(previewDisplayed)
        });
        return;
    }
